function updateReviews(data) {
  ...
  document.querySelectorAll('.reviews_holder').forEach(holder => {
      // First, add is-loaded class immediately
  ...
  // Only update review lists if they exist
  ...
  if (reviewLists.length > 0) {
      const reviewsHTML = reviews.length > 0 
          ...
          `).join('')
          : '<div class="review_list-empty">Noch keine Bewertungen vorhanden.</div>';
  ...
  console.log('Reviews holders found:', document.querySelectorAll('.reviews_holder').length);
  console.log('Is-loaded classes added:', document.querySelectorAll('.reviews_holder.is-loaded').length);
}
...
// Make updateReviews available globally
