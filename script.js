        ...
        <div class="star_holder-top">
          ...
          <div class="review_list-comment">
            ...
            <div class="comment_wrapper">
              <div class="comment_element">${review.name.split(' ').map(n => n[0]).join('.')}.</div>
              <div>
                <div class="commenter_name">
                  <div class="text-size-regular">${review.name}</div>
                  ${review.verified_customer ? `
                    <div class="icon-embed-16 w-embed">
                      <svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 0 15 16" fill="none" preserveAspectRatio="xMidYMid meet" aria-hidden="true" role="img">
                ...
                <div class="text-size-regular op_50">${new Date(review.date).toLocaleDateString('de-DE', { year: 'numeric', month: 'long', day: 'numeric' })}</div>
              </div>
            </div>
          </div>
        </div>
